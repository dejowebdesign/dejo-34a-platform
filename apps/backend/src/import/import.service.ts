...
import {
  ...
  ImportService as LegalImportService,
  OfficialXmlSourceService,
  ParserService,
  SchedulerService,
  VersionService
...
export class ImportService {
  ...
  private readonly importer = new LegalImportService(
    new OfficialXmlSourceService(),
    new ParserService(),
    this.changes
  );
  ...
  history(lawId?: string) {
    return this.prisma.importJob.findMany({
      where: lawId ? { lawId } : undefined,
      include: { law: true },
      orderBy: { startedAt: 'desc' }
    });
  }
  async status() {
    return {
      ...
      jobs: await this.prisma.importJob.findMany({
        include: { law: true },
        orderBy: { startedAt: 'desc' },
        take: 20
      })
  ...
  create(data: CreateImportJobDto) {
    return this.prisma.importJob.create({ data: this.toCreatePersistence(data) });
  }
  async update(id: string, data: UpdateImportJobDto) {
    await this.requireJob(id);
    return this.prisma.importJob.update({ where: { id }, data: this.toUpdatePersistence(data) });
  }
  ...
  async importManually(request: ManualImportDto) {
    ...
    if (!sourceUrl)
      throw new NotFoundException('No official XML source URL is configured for this law');
    const startedAt = new Date();
    const job = await this.prisma.importJob.create({
      data: { lawId: law.id, status: ImportJobStatus.RUNNING, startedAt }
    });
    ...
    try {
      ...
      const summary = await this.prisma.$transaction(async (transaction) => {
        ...
        const existingByNumber = new Map(
          existing.map((paragraph) => [paragraph.number, paragraph])
        );
        ...
        for (const parsed of document.paragraphs) {
          ...
          if (!current) {
            await transaction.paragraph.create({
              data: {
                lawId: law.id,
                ...
                versions: {
                  create: { version, text: parsed.text, checksum, sourceUrl, importedAt }
                }
              }
            });
            importedParagraphs++;
          } else if (this.changes.hasChanged(current.currentChecksum, checksum)) {
            await transaction.paragraph.update({
              where: { id: current.id },
              data: {
                title: parsed.title,
                currentText: parsed.text,
        ...
        const deletedParagraphs = existing.filter(
          (paragraph) => !checksums.has(paragraph.number)
        ).length;
      ...
      return this.prisma.importJob.update({
        ...
        data: {
          ...
          ...summary,
          warnings: summary.deletedParagraphs
            ? [
                `${summary.deletedParagraphs} paragraph(s) were absent from the source and retained for review.`
              ]
            : document.warnings,
          message: 'Official XML import completed.'
        },
        include: { law: true }
      });
    } catch (error: unknown) {
      ...
      return this.prisma.importJob.update({
        ...
        data: {
          ...
          duration: Date.now() - startedAt.getTime(),
          errors: [message],
          message: 'Official XML import failed.'
        },
        include: { law: true }
      });
    }
