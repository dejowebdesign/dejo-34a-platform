...
export class ImportController {
  ...
  @Get() @ApiOkResponse({ description: 'Lists import job records.' }) findAll() { return this.imports.findAll(); }
  @Get('status') @ApiOkResponse({ description: 'Returns current and recent import status.' }) status() { return this.imports.status(); }
  @Get('history') @ApiQuery({ name: 'lawId', required: false }) @ApiOkResponse({ description: 'Returns import history, optionally for one law.' }) history(@Query('lawId') lawId?: string) { return this.imports.history(lawId); }
  @Post('manual') @ApiOperation({ summary: 'Imports one law from its official XML or XML ZIP source.' }) @ApiCreatedResponse({ description: 'Runs a synchronous official XML import and returns its job record.' }) manual(@Body() data: ManualImportDto) { return this.imports.importManually(data); }
