...
export class ParagraphService {
  ...
  findAll() { return this.prisma.paragraph.findMany({ include: { law: true, subTopic: true }, orderBy: [{ law: { abbreviation: 'asc' } }, { number: 'asc' }] }); }
  async findOne(id: string) { return this.requireParagraph(id); }
  create(data: CreateParagraphDto) { return this.prisma.paragraph.create({ data }); }
  ...
  async updateVersion(id: string, data: UpdateParagraphVersionDto) {
    ...
    return this.prisma.paragraphVersion.update({ where: { id }, data: { ...rest, ...(importedAt ? { importedAt: new Date(importedAt) } : {}) } });
  }
  async removeVersion(id: string) { await this.requireVersion(id); return this.prisma.paragraphVersion.delete({ where: { id } }); }

  private async requireParagraph(id: string) {
    const paragraph = await this.prisma.paragraph.findUnique({ where: { id }, include: { law: true, subTopic: true, versions: { orderBy: { importedAt: 'desc' } } } });
    if (!paragraph) throw new NotFoundException('Paragraph not found');
    return paragraph;
  }
  private async requireVersion(id: string) {
    const version = await this.prisma.paragraphVersion.findUnique({ where: { id }, include: { paragraph: true } });
