...
export class ParagraphVersionController {
  ...
  @Get() @ApiQuery({ name: 'paragraphId', required: false }) @ApiOkResponse({ description: 'Lists paragraph versions, optionally for one paragraph.' }) findAll(@Query('paragraphId') paragraphId?: string) { return this.paragraphs.findAllVersions(paragraphId); }
