...
export class OfficialXmlSourceService implements ImportSource {
  async fetch(sourceUrl: string): Promise<string> {
    const response = await fetch(sourceUrl, { headers: { Accept: 'application/xml, application/zip' } });
  ...
  private extractXml(archive: Buffer): string {
    const entry = new AdmZip(archive).getEntries().find((candidate) => !candidate.isDirectory && candidate.entryName.toLowerCase().endsWith('.xml'));
